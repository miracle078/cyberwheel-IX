# Verbatim Viva Presentation Script
Adversarial Reinforcement Learning in Cybersecurity: Teaching RL Agents to Defend Against Cyber Attacks
Miracle Akanmode — August 2025

(Exact sequential reading script derived directly from `research_docs/cyberwheel_foundational_explanation.tex` with only minimal formatting adaptation for oral delivery. No paraphrasing. Equations and wording retained. Figure callouts inserted at original caption points. Environment labels (Foundation / Intuition / Mathematical Details / Example) are spoken cues.)
...
[Foundation]
Understanding the Battlefield: Before we learn how RL agents an learn to defend networks, we need to understand what a typical network looks like and what we're defending.
...
- Policy (\pi): The agent's strategy (which action to take in each state)
- Value Function (V): How good it is to be in a particular state

The Goal: Find a policy \pi that maximizes the expected return:
...
Where:
- T = total time steps
- \gamma = discount factor (0.95 in our research) - values future rewards less than immediate ones
...
Parameters: Learning rate $\alpha$, clipping parameter $\epsilon = 0.2$, GAE parameter $\lambda = 0.95$, minibatch size $M$, optimization epochs $K$
...
4. for epoch $= 1$ to $K$ do
   - for minibatch in $\{1, \ldots, N/M\}$ do
     - Optimize surrogate objective w.r.t. $\theta$:
...
[Example]
Key PPO Concepts:
- Probability Ratio: $r_t(\theta) = \pi_\theta(A_t|S_t) / \pi_{\theta_{\text{old}}}(A_t|S_t)$ measures how much the new policy differs from the old
...
Advantage Function: The advantage function $A^{\pi}(s,a) = Q^{\pi}(s,a) - V^{\pi}(s)$ measures the relative quality of action $a$ in state $s$ under policy $\pi$.
...
Input: Action spaces $\mathcal{A}^{(r)}, \mathcal{A}^{(b)}$, state space $\mathcal{S}$, balance threshold $\beta$
