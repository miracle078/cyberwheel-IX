...
We formulate adaptive cyber defense as a multi-agent reinforcement learning problem where defensive agents learn strategic deception through adversarial interaction. Using Proximal Policy Optimization, we train agents to deploy honeypots and decoys that mislead attackers and protect critical assets. Our key insight: adaptive placement creates strategic uncertainty, disrupting attack reconnaissance and lateral movement. 

Evaluated against traditional baselines across diverse networks with real MITRE ATT\&CK behaviors, our PPO-based approach achieves 34\% better asset protection than static defenses and 28\% improvement over rule-based systems, while maintaining enterprise deployment reliability. The learned policies generalize across network topologies, demonstrating robust defensive adaptation without human intervention.
\end{abstract}
...
I would like to express my sincere gratitude to Imperial College London and Imperial-X for providing the research infrastructure and academic support that made this work possible. Special appreciation goes to my supervisor Kelly Zhang for her guidance, and the cybersecurity research community whose foundational work in reinforcement learning and adversarial machine learning provided the theoretical framework for this investigation.
...
\textbf{AI Use Acknowledgement:} This research was conducted with assistance from Claude (Anthropic) for document analysis, literature review support, citation verification, and writing enhancement. The AI was used as a research tool for improving clarity and precision of technical communication, while all substantive research contributions, experimental design, implementation, and scientific conclusions remain the work of the author.
...
Contemporary cybersecurity faces an asymmetric challenge: defenders must protect all entry points while attackers need only one success. Security Operations Centers experience alert fatigue from massive volumes with high false positive rates \cite{sommer2010outside}.

\textit{Adaptive deception strategies} address this through strategic honeypot and decoy deployment that creates uncertainty for attackers while detecting threats \cite{anwar2022honeypot}. The SolarWinds supply chain attack (2019-2020) exemplifies this challenge, where threat actors maintained undetected access across approximately 18,000 organizations for months, demonstrating how advanced persistent threats systematically overcome static defenses through patient reconnaissance \cite{gao2021solarwinds}.

Current static mechanisms—firewall rules, signatures, manual procedures—cannot match adaptive adversaries \cite{apruzzese2022deep}. This necessitates intelligent, automated defensive responses that learn and adapt continuously.


\subsection{The Evolution of Cyber Defense: From Reactive to Adaptive}

Cybersecurity defense has evolved through three generations: \textbf{rule-based systems} using static signatures and patterns, \textbf{machine learning approaches} requiring labeled historical data, and \textbf{adaptive systems} enabling continuous learning without complete retraining \cite{alpcan2010network}.

Our research advances adaptive defense by integrating reinforcement learning with cyber deception. We train defender agents against dynamic attackers \cite{sutton2018reinforcement}, learning strategic decoy placement that creates uncertainty and disrupts attack reconnaissance (Figure~\ref{fig:defense_evolution}).

The next section examines how this research builds on and extends existing work in adversarial cybersecurity systems.
...
We establish a comprehensive three-category experimental methodology with statistical validation, providing a reproducible framework for future cybersecurity RL research. This includes algorithmic comparisons, environment studies, parameter analysis with multi-seed validation, HPC deployment protocols, and standardized evaluation metrics.
...
Cybersecurity defense evolved from game-theoretic foundations \cite{alpcan2010network} through machine learning approaches to reinforcement learning systems. Traditional ML methods improved threat detection but remained limited by dependence on labeled datasets and adaptation challenges \cite{sommer2010outside}. Adversarial vulnerabilities in deep learning \cite{goodfellow2014explaining} further motivated robust defensive mechanisms.

\subsection{Reinforcement Learning in Cybersecurity}

Reinforcement learning enables sequential decision-making under uncertainty \cite{sutton2018reinforcement}, well-suited for adaptive cyber defense. Key algorithmic advances include Deep Q-Networks \cite{mnih2015human}, Proximal Policy Optimization \cite{schulman2017proximal} which forms our algorithmic foundation, and multi-agent approaches \cite{lowe2017multi} for adversarial scenarios.

Recent cybersecurity RL applications evolved from single-agent intrusion detection \cite{zhu2021survey} to multi-agent adversarial systems. Oh et al. \cite{oh2023applying,oh2024employing} developed comprehensive RL frameworks for defensive agents and attack simulation, establishing important baselines but focusing primarily on single-agent scenarios.

\subsection{Multi-Agent Adversarial Systems}

Multi-agent RL addresses competitive cybersecurity scenarios through adversarial training dynamics \cite{tampuu2017multiagent}. MADDPG \cite{lowe2017multi} enables centralized training with decentralized execution, while self-play approaches \cite{openai2019dota} demonstrate effectiveness in competitive environments. Borchjes et al. \cite{borchjes2023adversarial} pioneered adversarial deep RL in software-defined networks using multi-agent games.
...
Farooq and Kunz \cite{farooq2025combining} recently combined supervised and reinforcement learning for generic defensive agents, while cyber deception research explored optimal honeypot allocation \cite{zhang2022optimal,anwar2022honeypot} and game-theoretic foundations \cite{zhu2021survey}.

\subsection{Research Gaps and Our Contributions}

Existing work exhibits key limitations: (1) single-agent focus without adversarial interaction, (2) limited deception strategy analysis, (3) evaluation on simple network topologies, and (4) lack of comprehensive baseline comparisons. Our research addresses these gaps through multi-agent adversarial training with strategic deception, comprehensive evaluation across diverse networks, and systematic comparison against multiple defensive baselines using real MITRE ATT\&CK behaviors.

Current approaches predominantly use static decoy deployment and rule-based management, limiting adaptability and strategic learning. This motivates RL-based adaptive systems that can dynamically optimize deployment strategies and discover sophisticated deception tactics.
...
Having established the theoretical foundations and identified research gaps, we now formalize our multi-agent reinforcement learning approach to adaptive cyber defense.
...
The blue agent's reward combines attack outcomes and action costs:
\begin{equation}
R_t^{(b)} = R_{\text{attack}}^{(b)} + R_{\text{action}}^{(b)}
\end{equation}

where the attack reward uses indicator functions for compact representation:
\begin{equation}
...
where $v_r$ represents the red agent's reward value for the current attack type: $v_r \in \{1, 2, 5, 10, 20, 50\}$ for pingsweep, portscan, discovery, lateral-movement, privilege-escalation, and impact phases respectively, calibrated to MITRE ATT\&CK severity progression.

\subsubsection{Reward Structure Properties}

The reward structure incentivizes strategic deception through a 10× multiplier for successful decoy attacks, high deployment costs (-20) that encourage careful placement, and scaled penalties matching attack severity. The blue agent optimizes the expected cumulative reward:
\begin{equation}
J^{(b)}(\pi^{(b)}) = \mathbb{E}_{\pi^{(b)}, \pi^{(r)}_{\text{fixed}}}\left[\sum_{t=0}^{H-1} \gamma^t R_t^{(b)}\right]
\end{equation}
where $\gamma = 0.99$ is the discount factor and $H$ is the episode horizon, optimized using PPO.
...
Each timestep follows: blue agent acts → red agent attacks → environment updates → rewards calculated → observations updated for next timestep.
...
\medskip
\begin{tikzpicture}[node distance=2.2cm,box/.style={draw=blue!60,rounded corners,fill=white,minimum width=3.4cm,minimum height=1.4cm,align=center,font=\small}]
    \node[box,fill=blue!15] (cat1) {\textbf{Category 1}\\Algorithm Comparison\\PPO vs Baselines\\Fixed Env (15 hosts)};
    \node[box,fill=orange!15,right=3.2cm of cat1] (cat2) {\textbf{Category 2}\\Environment Studies\\Vary Size / Topology\\Same PPO Config};
    \node[box,fill=red!15,below=2.0cm of cat2] (cat3) {\textbf{Category 3}\\Parameter Analysis\\LR / Entropy / Reward\\Fixed Env + PPO};
    \draw[->,very thick,blue] (cat1) -- node[above,font=\footnotesize]{Scale Dimension} (cat2);
    \draw[->,very thick,red] (cat2) -- node[right,font=\footnotesize]{Sensitivity} (cat3);
    ...
    \draw[->,thick,gray] (cat1.south) -- (controls.north);
    \draw[->,thick,gray] (cat3.south) -- (outputs.north);
\end{tikzpicture}
...
\paragraph{Performance Rankings}: Results demonstrate clear algorithmic effectiveness:
\begin{enumerate}
...
\begin{itemize}
\item \textbf{Strategic Learning}: PPO demonstrates strategic resource allocation with superior consistency compared to heuristic approaches
\item \textbf{Performance Hierarchy}: Clear separation between learned and rule-based approaches validates experimental methodology
\item \textbf{Baseline Validation}: Control baselines confirm the value of defensive actions and validate statistical framework
\end{itemize}
...
\paragraph{Standardized Evaluation Protocols}: The established three-category experimental methodology and evaluation metrics provide standardized protocols for comparative research.
...
We demonstrated that PPO-based defensive agents achieve 34\% better asset protection than static defenses and 28\% improvement over rule-based systems while maintaining enterprise deployment reliability. Our comprehensive evaluation across diverse network topologies with real MITRE ATT\&CK behaviors validates the effectiveness of learned adaptive deception strategies.

\paragraph{Key Findings}:

Our systematic evaluation reveals: (1) strategic resource allocation emerges from adversarial training, with PPO learning sophisticated defensive patterns that outperform manual strategies, (2) adaptive deception creates meaningful uncertainty for attackers, disrupting reconnaissance and lateral movement phases, (3) learned policies generalize across network topologies (15-200 hosts) without degradation, and (4) consistency advantages make PPO suitable for operational deployment where reliability is critical.

\paragraph{Scientific Contributions}:

The significant performance gaps between algorithms confirm that algorithmic choice is critical in cybersecurity RL. Our systematic methodology provides: statistical validation through multi-seed trials, comprehensive baseline comparisons establishing the value of defensive actions, and reproducible experimental protocols for future research.

\paragraph{Implications}:

This research establishes that adaptive deception through RL offers measurable advantages over traditional approaches. The demonstrated generalization across network scales and consistent performance characteristics provide a foundation for practical deployment in enterprise cybersecurity operations.

Future work should investigate adversarial robustness against adaptive attackers, integration with existing security infrastructures, and extension to novel attack vectors. Our experimental framework and baseline comparison methodology enable systematic evaluation of these extensions with confidence in scientific rigor.
