...
\newcommand{\reporttitle}{Toward Intelligent Cyber Defense Through Reinforcement Learning: An Experimental Study of Adaptive Decoy Placement in Multi-Agent Adversarial Environments}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\tikzset{
    vectorbox/.style={draw=black, rounded corners, fill=white, inner sep=8pt, minimum width=4.0cm, align=center, thick},
    obssection/.style={draw=black, rounded corners, fill=blue!8, inner sep=8pt, minimum width=9cm, align=left, thick},
...
\end{tikzpicture}
\caption{Evolution of Cyber Defense: From static rule-based systems to dynamic adversarial learning approaches.}
...
\begin{itemize}
    ...
    \item Optimize resource allocation between detection and deception objectives
\end{itemize}

represents a significant advancement over current manual and rule-based approaches.

\section{Mathematical Problem Formulation}

This section establishes the complete mathematical framework for our adversarial reinforcement learning approach to cyber defense. We provide clear variable definitions and formalize the multi-agent game-theoretic setting.
...
\begin{itemize}
    ...
    \item $A^{(r)}_t$: Red agent action at time $t$
    \item $A^{(b)}_t$: Blue agent action at time $t$
    \item $d_r, d_b$: Dimensions of red and blue state spaces respectively
\end{itemize}

\textbf{Learning and Policy Variables:}
\begin{itemize}
    \item $\pi^{(r)}_\theta: S^{(r)} \to \Delta(A^{(r)})$: Red agent policy with parameters $\theta$
...
\begin{itemize}
    ...
    \item $\mathcal{E}_t \subset \mathcal{T}$: Subset of techniques executable by red agent in state $S^{(r)}_t$
    \item $\text{phase}_t \in \{\text{discovery}, \text{reconnaissance}, \text{privilege-escalation}, \text{impact}\}$: Current attack phase
\end{itemize}
...
\begin{itemize}
    ...
    \item $\gamma \in [0,1)$: Discount factor (set to 0.95 in our implementation)
...
\textit{Multi-step Effects}: While immediate rewards capture direct action consequences, our reward structure also accounts for delayed effects through the temporal discount factor $\gamma = 0.95$, allowing agents to learn multi-step strategies where early actions enable later tactical advantages.
...
\textbf{Experimental Design}:
\begin{itemize}
\item \textbf{Fixed Algorithm}: PPO with verified hyperparameters (lr=3e-4, entropy=0.01)
